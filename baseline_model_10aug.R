...
#saving the dataset
write.csv(all_model_stats, file = "baseline_model.csv", row.names = FALSE)
all_model_stats <- read.csv("baseline_model.csv")
...
tab_model(compare_model, show.re.var = TRUE, show.icc = TRUE, show.r2 = TRUE)
tab_model(compare_model, file = "baseline_model_summary.html")
...
# Plot the model predictions
ggplot(long_data_mls, aes(x = Measure, y = MeanLeaveTimes, fill = Measure)) +
  geom_boxplot() +
  labs(x = "Measure", y = "Mean Leave Times", fill = "Measure") +
  theme_bw() +
  theme(
