...
## USING A 10:1 PAIRS TO FAMILY RATIO TO SIMULATE POOR BREEDING YEAR
Fs <- c(4,2,8)
Ps <- c(42, 21, 84)
#As long as the no. of singles is small it's biologically plausible
...
sp_mat<-dist_mat<tc ###THINK THIS ONE WORKS 
...
all_model_stats <- merge(combined_dyad_stats, combined_hyp_stats, 
                         by = c("F", "P", "S", "replicate_no", "space_replicate", "t", "tc"), 
...
#saving the dataset to work with in future
write.csv(all_model_stats, file = "pby_data_nosingle_final.csv", row.names = FALSE)

######################
all_model_stats<-read.csv("pby_data_nosingle_final.csv")
library(lme4) #lme4 models

#hypergraph variance -> random effects model
hyp_variance_model <- lmer(hyp_mean_leave_times ~ 1 + (1|fam_combo) + (1|t) + (1|tc) + (1|replicate_unique), data = all_model_stats)
summary(hyp_variance_model)

#dyadic variance -> random effects model
dyad_variance_model <- lmer(dyad_mean_leave_times ~ 1 + (1|fam_combo) + (1|t) + (1|tc) + (1|replicate_unique), data = all_model_stats)
...
hyp_mod2<-lmer(hyp_mean_leave_times ~ as.factor(fam_combo) + as.factor(t) + as.factor(tc) + (1|replicate_unique), data = all_model_stats)
summary(hyp_mod2)
#calculating the replicate variance
hyp_rep<-r.squaredGLMM(hyp_mod2)
re_var_hyp<-hyp_rep[2]-hyp_rep[1]

dyad_mod2<-lmer(dyad_mean_leave_times ~ as.factor(fam_combo) + as.factor(t) + as.factor(tc) + (1|replicate_unique), data = all_model_stats)
summary(dyad_mod2)
...
dyad_partR2_data <- data.frame(
  ...
  R2 = c(0.237, 0.0540, 0.0899, 0.0928, 0.144, 0.147, 0.183, 0.237)
)
...
hyp_partR2_data <- data.frame(
  Predictor = c("Model", "as.factor(fam_combo)", "as.factor(t)", "as.factor(tc)", "as.factor(fam_combo)+as.factor(t)", 
                "as.factor(fam_combo)+as.factor(tc)", "as.factor(t)+as.factor(tc)", "as.factor(fam_combo)+as.factor(t)+as.factor(tc)"),
  R2 = c(0.383, 0.0952, 0.245, 0.0429, 0.340, 0.138, 0.288, 0.383)
)
