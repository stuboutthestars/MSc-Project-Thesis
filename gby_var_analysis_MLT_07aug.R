...
dyad_partR2_data <- data.frame(
  ...
  R2 = c(0.895, 0.0438, 0.446, 0.00117, 0.0883, 0.0449, 0.457, 0.0895)
)
...
hyp_partR2_data <- data.frame(
  Predictor = c("Model", "as.factor(fam_combo)", "as.factor(t)", "as.factor(tc)", "as.factor(fam_combo)+as.factor(t)", 
                "as.factor(fam_combo)+as.factor(tc)", "as.factor(t)+as.factor(tc)", "as.factor(fam_combo)+as.factor(t)+as.factor(tc)"),
  R2 = c(0.322, 0.189, 0.0946, 0.0382, 0.284, 0.228, .133, 0.322)
)
...
hist(all_model_stats$dyad_mean_leave_times, 
     ...
     border = NA, 
     breaks = breaks_seq_mlt, 
     ylim = c(0, 700), 
     xlim = c(0, 60),
     xlab = "Mean Leave Time (timesteps)",
     ylab = "Frequency",
     cex.main = 1.5, 
     cex.lab = 1.2, 
     cex.axis = 1.1,
...
tab_model(compare_model_mlt, file = "baseline_model_summary_mlt.html")
