...
#Adding back the network graphs for display and preparing final aim models
...
# Convert adjacency matrix to igraph object - #addin the diag bit elsewhere if needed
sp_net_graph <- graph_from_adjacency_matrix(sp_net, mode = "undirected",diag=FALSE)
...
degree_centrality <- degree(sp_net_graph)

starters<-2

  hc<-max(degree_centrality)
  mc<-which(degree_centrality==hc)
  if(length(mc)>1){
    initial_callers<-sample(mc,2,replace=FALSE)
  }
  if(length(mc)==1){
    i1<-which.max(degree_centrality)
    dc2<-degree_centrality[-i1]
    hc2<-max(dc2)
    mc2<- which(dc2==max(dc2))
    if(length(mc2)>1){
      t_i2<-sample(mc2,1,replace=FALSE)
      i2<-as.numeric(names(dc2)[t_i2])
    }
    if(length(mc2)==1){
      i2<-as.numeric(names(dc2)[mc2])
    }
    initial_callers<-c(i1,i2)
  }
  
#Code to calculate overlap in networks between the two initial callers
ego<-ego(sp_net_graph,order=1,nodes=initial_callers)
overlap<-sum(ego[[1]]%in%ego[[2]])/length(ego[[1]])
subgroup_overlap[i] <- overlap

# Sample initial callers based on degree centrality
#initial_callers <- sample(1:nrow(sp_mat), 2, prob = degree_centrality_norm)
...
#currently set up to use the dyadic degree centrality individuals 
callers<-og_callers
...
#saving the dataset
#write.csv(all_model_stats, file = "social_structure_replicated_03aug.csv", row.names = FALSE)

all_model_stats <- read.csv("social_structure_replicated_03aug.csv")
...
breaks_seq_mlt2 <- seq(min_leave_times, max_leave_times, length.out = 50)
